...
This project is a modular, scalable, and easy-to-maintain **API Automation Framework** built using **RestAssured**. 
It is designed for validating RESTful APIs and includes robust logging, reporting, and integration capabilities to support CI/CD pipelines.
...
| Logging          | Log4j, Apache POI                 |
| Reporting        | Allure Report                     |
| Code Quality     | SonarLint                         |
| Version Control  | Git                               |
| Remote Repo      | GitHub                            |
| CI/CD Tool       | Jenkins                           |
| JSON Libraries   | Jackson API / Gson                |
| AssertJ          | Assertions                        |
...
├── main
│ └── java
│ └── utils # Utility classes (Payload manager, Config, etc.)
├── test
│ └── java
│ └── tests # Test classes
...
SonarLint plugin is used to maintain code quality. Install it in your IDE (IntelliJ / Eclipse) and it will highlight issues as you code.

🔁 CI/CD Integration
GitHub: Push all your code regularly to a remote repository.

Jenkins:
Configure a Maven Job.
Pull from GitHub.
Run the tests with mvn clean test.
Publish Allure Reports using Allure Jenkins Plugin.


📌 Best Practices
Keep test data and payloads separate from test logic.
Use POJO classes for payload modeling.
Enable logging using log4j.properties for debugging.
Follow naming conventions and group tests using @Test(groups = "smoke") etc.
Clean up resources in @AfterClass or @AfterMethod where necessary.
